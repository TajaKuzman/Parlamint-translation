...
# Define final path
notes_path = "/home/tajak/Parlamint-translation/Note-translation/Final-data-CSV/ParlaMint-{}.notes.translated.csv".format(lang_code)

temp_path = "/home/tajak/Parlamint-translation/Note-translation/Final-data-CSV/ParlaMint-{}.notes.translated-before-sorting.csv".format(lang_code)
...
def extract_tag(tag, df, content):
	if tag in ["note", "head"]:
		...
		for i in note_list:
			current_note = []
			# Append the attribute value (tag type) if there is only one attribute
			if len(list(i.attrs.values())) == 1:
				current_note.append(list(i.attrs.values())[0])
			# If the tag does not have attributes, append empty string
			elif len(list(i.attrs.values())) == 0:
				current_note.append("")
			# Print out an error notification if the tag has more than 1 attribute
			else:
				print("Error: there are more than 1 attribute!")

			# Add a possibility that there would be lang attribute in the tag
			if "xml:lang" in list(i.attrs.keys()):
				lang = i.get("xml:lang")
			else:
				lang = ""
			# Append information about content of the tag and language
			current_note.append(i.get_text())
			current_note.append(lang)
			note_list_final.append(current_note)
		
		# Create a df with all the information from the tags
		...
		# Notes other than head and note have text inside the <desc> tag and there could be more than one such tag inside them
		for i in note_list:
			desc_list = []
			if len(list(i.attrs.values())) == 1:
				current_type = list(i.attrs.values())[0]
			elif len(list(i.attrs.values())) == 0:
				current_type = current_note.append("")
			else:
				print("Error: there are more than 1 attribute!")
			...
			for desc in desc_list:
				...
				if "xml:lang" in list(desc.attrs.keys()):
					...
					lang = ""
				current_note.append(current_type)
...
def translate_notes(path, lang_code, opus_lang_code, notes_path, temp_path):
	...
	# Save the final df
	df.to_csv("{}".format(notes_path), sep="\t", index=False)
